 ...
 * Description: Built by developers, for developers. Display one or multiple locations with proper Schema.org markup and Google Maps to boot. 
 * Version: 0.0.1
 * Author: Matmon
...
function get_wpseo_locations($ids){
	...
	for($i=0;$i<$wp_locations_count;$i++){
		...
		$wp_locations[$i]->google_map_image_url = 'https://maps.googleapis.com/maps/api/staticmap?zoom='.$map_zoom.'&size='.$map_width.'x'.$map_height.'&maptype='.$map_type.'&format='.$image_type.'&markers=color:0x'.$pin_color.'|'.$latitude.','.$longitude;
